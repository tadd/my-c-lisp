...
static IntTable *name_to_symbol;
...
static IntTable *filename_to_newline_pos; // Value => array<uint64_t>
static IntTable *pair_to_function_location; // Value (Pair) => Location
...
{
    // find
    Symbol sym = (Symbol) int_table_get(name_to_symbol, (uint64_t) name);
    if (sym == 0) { // or put
        sym = next_symbol++;
        const char *s = xstrdup(name);
        int_table_put(name_to_symbol, (uint64_t) s, sym);
...
typedef struct {
    ...
    Token prev_token;
    IntTable *function_locations; // Value (Pair) => Location
...
static void record_location(Parser *p, Value pair, int64_t pos, Value sym)
{
    ...
    int_table_put(p->function_locations, pair, (uint64_t) loc);
}
...
static Parser *parser_new(FILE *in, const char *filename)
{
    ...
    p->function_locations = int_table_new();
...
{
    int_table_merge(pair_to_function_location, p->function_locations);
    int_table_put(filename_to_newline_pos, value_of_symbol(p->filename),
...
static void dump_line_column(Value vfilename, uint64_t pos)
{
    const uint64_t *newline_pos =
        (uint64_t *) int_table_get(filename_to_newline_pos, vfilename);
    if (newline_pos == NULL) {
...
{
    return (Location *) int_table_get(pair_to_function_location, pair);
}
...
static void initialize(void)
{
    ...
    next_symbol = 1;
    name_to_symbol = int_table_new_str();
    ...
    filename_to_newline_pos = int_table_new();
    pair_to_function_location = int_table_new();
