...
static IntTable *filename_to_newline_pos; // Value (Pair) => array<uint64_t>
static IntTable *pair_id_to_function_location; // Value (Pair) => Location
...
static void record_location(Parser *p, Value pair, int64_t pos, Value sym)
{
    ...
    int_table_put(p->function_locations, pair, (uintptr_t) loc);
}
...
{
    return (Location *) int_table_get(pair_id_to_function_location, pair);
}
...
{
    int_table_merge(pair_id_to_function_location, tree->function_locations);
    uint64_t val = (uintptr_t) tree->newline_pos;
    int_table_put(filename_to_newline_pos, tree->filename, val);
}
...
static void initialize(void)
{
    ...
    pair_id_to_function_location = int_table_new();
