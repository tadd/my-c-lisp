...
//PTR
static Value apply_closure(Table *env, Value proc, Value args)
{
    ...
    bool got_oldp = false;
    if (cl->env != env) {
        oldp = table_set_parent(env, cl->env);
        got_oldp = true;
    }
...
// 4.1.4. Procedures
static Value lambda(Table *env, Value params, Value body)
{
