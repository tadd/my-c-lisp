...
Test(table, get_put) {
    ...
    int_table_put(t, 1, 100);
    cr_assert(eq(int, 100, int_table_get(t, 1)));
    int_table_put(t, 2, 200);
    int_table_put(t, 3, 300);
    int_table_put(t, 4, 400);
    cr_assert(eq(int, 100, int_table_get(t, 1)));
    cr_assert(eq(int, 200, int_table_get(t, 2)));
    cr_assert(eq(int, 300, int_table_get(t, 3)));
    cr_assert(eq(int, 400, int_table_get(t, 4)));
    int_table_put(t, 1, 42);
    cr_assert(eq(int, 42, int_table_get(t, 1)));
    cr_assert(eq(int, 200, int_table_get(t, 2)));
    cr_assert(eq(int, 300, int_table_get(t, 3)));
    cr_assert(eq(int, 400, int_table_get(t, 4)));

    for (int i = 1; i <= 17; i++)
        int_table_put(t, i, i*10000000);
    for (int i = 1; i <= 17; i++)
...
Test(table, custom_equal_func) {
    ...
    IntTable *t = int_table_new_full(strhash, streq);
    int_table_put(t, (uintptr_t) xstrdup(s[0]), 12);
    int_table_put(t, (uintptr_t) xstrdup(s[1]), 34);
    int_table_put(t, (uintptr_t) xstrdup(s[0]), 56);

    cr_assert(eq(int, 56, int_table_get(t, (uintptr_t) "foo")));
    cr_assert(eq(int, 34, int_table_get(t, (uintptr_t) "bar")));
