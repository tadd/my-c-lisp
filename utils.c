...
static inline void rapid_mum(uint64_t *a, uint64_t *b)
{
    ...
    uint64_t t = cl + (cm0 << 32U);
    uint64_t rl = t + (cm1 << 32U);
...
static uint64_t int_table_hash(uint64_t x)
{
...
static inline bool int_table_too_many_elements(const IntTable *t)
{
...
static void int_table_resize(IntTable *t)
{
